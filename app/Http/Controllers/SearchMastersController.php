...
class SearchMastersController extends Controller
{
    ...
    {
        if(Auth::guest()){
            $masters = User::with('ratings')->where('type','master')->latest()->take(10)->get();
        }else{
            $masters = User::with('ratings')->where('type','master')
    ...
    {
        $masters = User::with('ratings')->where('type','master')->where('category_id', $id)->get();
        return $masters;
    }
